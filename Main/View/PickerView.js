    ...
    shouldComponentUpdate(nextProps, nextState) {
      if (nextProps) {
        ...
        if (listChange || indexChange) {
          ...
          this.onSeleted(Math.abs((-this.props.itemHeight * nextProps.selectedIndex) / this.props.itemHeight - 2));
          // if (listChange) {
          this.colorPath = [];
          let length = list.length;
          for (let i = 0; i < length; i++) {
    ...
    renderItem(item, index) {
      return <View
        ...
        <Animated.Text style={{
          ...
          }), fontSize: this.props.fontSize ? this.props.fontSize : this.getSize(18),
