...
export default class MHDatePicker extends React.Component {
     ...
     */
    slice(arr, head, tail) {
      if (parseInt(head) == 1 && parseInt(tail) == 9) {
            ...
            return (
              <View
                ...
                <StringSpinner
                  ...
                  defaultValue={parseInt(currentArray[index]) > 0 ? `${ parseInt(currentArray[index]) }` : currentArray[index]}
    ...
    _onValueChanged(index, data) {
      ...
      this.setState({
        ...
        this.setState({ unit: HomeLocalizableString.s });
        if (this.props.type === TYPE.DATE) {
