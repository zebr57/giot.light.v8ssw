...
let b;
let t;
export default class SubAwake extends BaseComponent {
    static navigationOptions = ({ navigation }) => {
      return {
        header: <NavigationBar
          backgroundColor={navigation.state.params ? navigation.state.params.backgroundColor : DarkMode.getColorScheme() === 'dark' ? 'xm#000' : '#ffffff'}
          type={NavigationBar.TYPE.LIGHT}
          title={LocalizableString.唤醒模式}
          left={[
            {
              key: 'back',
    ...
    _getProps() {
      Service.spec.getPropertiesValue(getPropsPara, 1)
        .then((res) => { // 请求成功
          ...
          this.setState({
            awake_time: res[0].value,
            awake: res[1].value
          });

          if (res[2].code == 0) {
            this.setState({ endBright: res[2].value });
          }

          if (res[3].code == 0) {
            this.setState({ endColor: res[3].value });
          }
        });
    }
    ...
    _getPropsSpecBle4() {
      ...
      Bluetooth.spec.getPropertiesValue(Device.mac, json).then((data) => {
        ...
        if (jsonData.objects[0].code == 0) {
          this.setState({
            endColor: jsonData.objects[0].value
          });
        } else {
          this.setState({ loadingVis: false });
        }
    ...
    _getPropsSpecBle3() {
      ...
      Bluetooth.spec.getPropertiesValue(Device.mac, json).then((data) => {
        ...
        if (jsonData.objects[0].code == 0) {
          this.setState({
            endBright: jsonData.objects[0].value
          });
    ...
    _getPropsSpecBle2() {
      ...
      Bluetooth.spec.getPropertiesValue(Device.mac, json).then((data) => {
        ...
        if (jsonData.objects[0].code == 0) {
          this.setState({
            awake: jsonData.objects[0].value
          });
    ...
    _getPropsSpecBle1() {
      ...
      Bluetooth.spec.getPropertiesValue(Device.mac, json).then((data) => {
        ...
        if (jsonData.objects[0].code == 0) {
          this.setState({
            awake_time: jsonData.objects[0].value
          });
      ...
      if (this.state.btConnect) {
        this.listenerBle = DeviceEvent.BLESpecNotifyActionEvent.addListener((device, result) => {
          result.forEach((key, value) => {
            ...
            if (value == 'prop.4.12') {
              this.setState({ awake: key });
            }
            if (value == 'prop.4.11') {
              this.setState({ awake_time: key });
            }
            if (value == 'prop.4.13') {
              this.setState({ endBright: key });
            }
            if (value == 'prop.4.14') {
              this.setState({ endColor: key });
            }
          });
        });
        ...
        this.listener = DeviceEvent.deviceReceivedMessages.addListener((device, messages) => {
          if (messages.has('prop.4.12')) {
            this.setState({
              awake: messages.get('prop.4.12')[0]
            });
          }
          if (messages.has('prop.4.11')) {
            this.setState({
              awake_time: messages.get('prop.4.11')[0]
            });
          }
          if (messages.has('prop.4.13')) {
            this.setState({
              endBright: messages.get('prop.4.13')[0]
            });
          }
          if (messages.has('prop.4.14')) {
            this.setState({
              endColor: messages.get('prop.4.14')[0]
            });
          }
        }
        );
        ...
        Device.getDeviceWifi().subscribeMessages(
          'prop.4.11', 'prop.4.12', 'prop.4.13', 'prop.4.14')
          .then((subcription) => {
          ...
          <AbstractDialog
            ...
            buttons={[
              ...
              },
              {
                ...
                text: HomeLocalizableString.ensure,
                callback: (result) => {
                  this._sendCode(4, 11, this.state.temp_awake_time);
